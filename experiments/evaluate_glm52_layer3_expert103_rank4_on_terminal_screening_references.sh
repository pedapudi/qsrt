...
# Evaluate the frozen layer-3 expert-103 rank-four correction on the eight
# bounded endpoint references. This script is an explicit launch only; no
# capture or download process invokes it automatically.
...
RESULT_NAME="glm52-layer3-expert103-rank4-terminal-endpoint-eight-document-screen"
ARTIFACT_ROOT="${EXPERIMENT_ROOT}/results/${ARTIFACT_NAME}"
RESULT_ROOT="${EXPERIMENT_ROOT}/results/${RESULT_NAME}"
REFERENCE_DIRECTORY="${REFERENCE_ROOT}/screening-logits"
...
if [[ -n "${gpu_processes}" ]]; then
  echo "terminal-reference screening requires four idle GPUs" >&2
  exit 1
fi

read -r artifact_manifest_sha256 first_reference < <(
  python3 - "${ARTIFACT_ROOT}/report.json" "${REFERENCE_DIRECTORY}/manifest.json" <<'PY'
import json
import sys
...
assert references["status"] == "available_for_candidate_screening"
assert references["generation_contract"]["evaluation_tier"] == "screening"
assert references["document_count"] == 8
assert references["total_logit_rows"] == 9334
...
docker create \
  --name "${CONTAINER_NAME}" \
  --label qsrt.experiment="glm52-terminal-endpoint-eight-document-candidate-screen" \
  --label qsrt.model-downloads-performed=false \
  --label qsrt.reference-document-count=8 \
...
assert report["evaluation_tier"] == "screening"
assert report["measurement_controls"]["passed"] is True
assert report["summary"]["document_count"] == 8
assert report["summary"]["position_count"] == 9334
assert report["model_downloads_performed"] is False
completion = {
    "schema": "qsrt_glm52_terminal_endpoint_candidate_screen_completion",
    "schema_version": 1,
    "status": "complete",
