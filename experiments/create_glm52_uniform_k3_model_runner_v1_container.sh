...
# Create one network-isolated vLLM process that compares the resident EXL3
# checkpoint with dense uniform-K3 replacements for the frozen layer-3 panel.
# Routed-expert capture requires vLLM Model Runner V1 in the pinned v39 image.
...
mkdir -p "${record_dir}" "${experiment_root}/captures" "${experiment_root}/results"
test ! -e "${capture_path}"
test ! -e "${result_path}"
...
container_id="$(
  docker create \
    ...
    --label qsrt.copy-validation-required-before-start=true \
    --network none \
    --gpus all \
    --ipc host \
    --shm-size 64g \
    ...
    -e QSRT_GLM52_ACTIVATION_CAPTURE_DIR=/captures/glm52-layer3-wikitext-document-disjoint-routed-inputs \
    -e QSRT_GLM52_ACTIVATION_CAPTURE_PLAN_SHA256=b694ac0a1aeb09f7c61a20b5f72289f3e791d616ff7471b5894d857f8c363b55 \
    -e HF_HOME=/hf-corpus \
    ...
    --dest /results/glm52-layer3-frozen8-uniform-k3-paired-bf16-reference-kld-model-runner-v1 \
    --corpus-plan /workspace/qsrt/experiments/glm52_wikitext_document_disjoint_corpus_plan.json \
    --activation-capture-dir /captures/glm52-layer3-wikitext-document-disjoint-routed-inputs \
    --context-length 2048 \
