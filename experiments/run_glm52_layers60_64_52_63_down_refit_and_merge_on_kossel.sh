...
# Refit each frozen panel's down projections against that panel's own QSRT-K3
# gate/up reconstructions. Ridge strength is fit and selected on document-
# disjoint routed rows captured from the unchanged resident model.
...
for layer in 60 64 52 63; do
  ...
  input_name="glm52-layer${layer}-frozen8-uniform-k3-source-target-capture-sequenced-merged"
  capture_root="${capture_parent}/layer-$(printf '%03d' "${layer}")"
  slice_stem="glm52-layer${layer}-frozen8-reconstructed-activation-down-refit-slice"
  merged_name="glm52-layer${layer}-frozen8-reconstructed-activation-down-refit-merged"
  ...
  for gpu in 0 1 2 3; do
    ...
    docker create \
      --name "${container_name}" \
      --label qsrt.experiment="glm52-layer${layer}-reconstructed-activation-down-refit" \
  ...
  docker create \
    ...
    --label qsrt.experiment="glm52-layer${layer}-reconstructed-activation-down-refit-merge" \
    --label qsrt.model-downloads-performed=false \
    --network none \
    --entrypoint /opt/venv/bin/python \
