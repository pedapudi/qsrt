...
# Test the EXL3 R7 control kernel under dense attention. GLM-5.2's sparse top-k
# is 2,048, so dense attention admits the same complete causal key set at this
# bounded context. VLLM_EXL3_R7_FUSED=0 selects the image's documented
# exl3_moe_r7_fused control instead of SparkInfer's mixed-Trellis MoE kernel.
...
runtime_cache="${experiment_root}/runtime-cache/glm52-exl3-r7-control-dense-triton-bf16-reference-kld"
...
container_id="$(
  docker create \
    ...
    --dest /results/glm52-layer3-frozen8-uniform-k3-paired-bf16-reference-kld-dense-triton-r7-control-repeat-direct-identity \
    --context-length 2048 \
