...
# Build the same uniform QSRT-K3 control for two claimed high-impact GLM-5.2
# layers and two nearby controls. Each layer uses four independent two-expert
# workers, followed by a byte-preserving merge in frozen panel order.
...
capture_index="${experiment_root}/captures/glm52-layers52-60-63-64-wikitext-document-disjoint-routed-inputs-artifact-layer-compatible/manifest.json"
...
test -f "${source_snapshot}/scripts/build_glm52_dense_expert_interventions.py"
test -f "${capture_index}"
docker image inspect "${image}" >/dev/null

for layer in 60 64 52 63; do
  panel="glm52_layer${layer}_rate_pattern_panel.json"
  slice_stem="glm52-layer${layer}-frozen8-uniform-k3-source-target-capture-sequenced-slice"
  merged_name="glm52-layer${layer}-frozen8-uniform-k3-source-target-capture-sequenced-merged"
  ...
  for gpu in 0 1 2 3; do
    ...
    docker create \
      --name "${container_name}" \
      --label qsrt.experiment="glm52-layer${layer}-uniform-k3-source-target-capture-sequenced" \
  ...
  docker create \
    ...
    --label qsrt.experiment="glm52-layer${layer}-uniform-k3-source-target-capture-sequenced-merge" \
    --label qsrt.model-downloads-performed=false \
    --network none \
    --entrypoint /opt/venv/bin/python \
