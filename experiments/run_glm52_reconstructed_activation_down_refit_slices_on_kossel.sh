...
# Fit down-projection corrections on routed activation-fit documents, select
# ridge strength on separate documents, and re-encode four disjoint slices.
...
record_root="${experiment_root}/launch-records/glm52-layer3-reconstructed-activation-down-refit"
...
for gpu in 0 1 2 3; do
  ...
  slice_name="glm52-layer3-frozen8-reconstructed-activation-down-refit-slice-0${offset}-0${end}"
  ...
  docker create \
    --name "${container_name}" \
    --label qsrt.experiment=glm52-layer3-reconstructed-activation-down-refit \
