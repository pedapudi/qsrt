...
The packed side invokes the production B12X W4A16 MoE preparation and
execution path. The reference side decodes the same rank-local payload into
BF16 weight matrices and replays the coupled activation-boundary transform.
Inputs are real routed-MoE rows from an all-row capture; each sampled row is
evaluated as a single-expert route to isolate packed expert execution.
"""
...
B12X_ROOT = Path("/home/luke/projects/b12x")
sys.path.insert(0, str(B12X_ROOT))

from benchmarks.benchmark_qsrt_checkpoint_profiles import (  # noqa: E402
    _K2,
    _prepare_profile,
    _read_layer_source,
)
...
from b12x.moe._shared.kernels.w4a16.kernel import run_w4a16_moe  # noqa: E402
from tests.moe.test_fused_moe_trellis import (  # noqa: E402
    _reconstruct_native,
...
def _reference_output(
    ...
    intermediate = int(prepared.intermediate_size)
...
def main() -> None:
    ...
    if args.summary_only:
        print(
            json.dumps(
                {name: value for name, value in report.items() if name != "tiles"},
                indent=2,
                sort_keys=True,
            )
        )
